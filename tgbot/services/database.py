...
class DataBase:
    ...
    def add_card(self, user_tg_id: int, title: str, number: str, service_name: str):
        ...
        if not logo_id:
            ...
            self.__execute_query(
                'INSERT INTO Cards (user_tg_id, title, number, logo) VALUES (%s, %s, %s, %s);',
                (user_tg_id, title, number, logo_id)
            )
            return True

    def get_card(self, user_tg_id: int, card_id) -> tuple:
        """ Додавання карти/гаманця користувача в базу даних """
        return self.__execute_query(
            'SELECT Cards.id, Cards.title, Cards.number, Logos.url FROM Cards INNER JOIN Logos ON Cards.logo=Logos.id '
            'WHERE user_tg_id = %s AND Cards.id = %s;',
            (user_tg_id, card_id)
        )[0]

    def get_all_cards(self, user_tg_id: int):
    ...
    def update_card_logo(self, card_id: int, service_name: str):
